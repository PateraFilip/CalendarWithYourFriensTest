...
const API_URL_CREATE_WEEKLY =
  'https://tzbpcbmxwbsixrtorijk.supabase.co/functions/v1/add_weekly_event'
...
export const createWeeklyEvent = async (event: CreateWeeklyEventInput) => {
  const response = await fetch(API_URL_CREATE_WEEKLY, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: API_KEY,
